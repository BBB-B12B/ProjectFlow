...
export function TaskPrioritizationMatrix({
    ...
    const handleScatterClick = (data: any) => {
        if (data.taskCount > 1) {
            // Open Dialog for groups
            setActiveGroup(data);
        } else if (setSelectedProjectId && data.projectId) {
            // Select Project for single task
        ...
        <>
            <Card className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl border transition-all duration-200 hover:shadow-lg ${isHighlighted ? 'ring-2 ring-purple-400' : ''}`}>
                <CardHeader>
                    ...
                    <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Click points to select project. Click labels to filter quadrant.</p>
                </CardHeader>
                <CardContent>
                    <div className="h-80">
                                    ...
                                    content={({ active, payload }) => {
                                        ...
                                        return (
                                            <div className={`${isDark ? 'bg-gray-700 border-gray-600 text-gray-100' : 'bg-white border-gray-200 text-gray-800'} rounded-lg shadow-lg p-3 min-w-[200px]`}>
                                                ...
                                                <div className={`text-xs px-2 py-1 rounded inline-block my-1 ${data.quadrant === 'quick-wins' ? 'bg-green-100 text-green-800' : data.quadrant === 'major-projects' ? 'bg-yellow-100 text-yellow-800' : data.quadrant === 'fill-ins' ? 'bg-gray-100 text-gray-800' : 'bg-red-100 text-red-800'}`}>
                                                ...
                                                {data.taskCount === 1 ? (
                                                    <>
                                                        ...
                                                        <div className="text-xs text-blue-600 dark:text-blue-400 mt-1 font-medium">Click to select project</div>
                                                    </>
                                                ) : (
                                                    <>
                                                        ...
                                                        <div className="text-xs text-blue-600 dark:text-blue-400 mt-1 font-medium">Click to view all tasks</div>
                                                    </>
                                                )}
                                            </div>
                                        );
                                ...
                                >
                                    {scatterData.map((entry, index) => (
                                        <Cell
                                            ...
                                            style={{
                                                opacity: entry.isSelected ? 1 : (selectedQuadrant ? 0.3 : 1),
            ...
            <Dialog open={!!activeGroup} onOpenChange={(open) => !open && setActiveGroup(null)}>
                <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>Tasks at Effort {activeGroup?.effort} / Effect {activeGroup?.effect}</DialogTitle>
                        <DialogDescription>
                            Select a task to filter the dashboard by its project.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-2 mt-4">
                        {activeGroup?.taskList?.map((task: any, index: number) => (
                            <div
                                key={index}
                                className="p-3 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-colors"
                                onClick={() => {
                                    if (setSelectedProjectId && task.projectId) {
                            ...
                            >
                                ...
                                <div className="text-sm text-gray-500 mt-1 flex justify-between">
                                    ...
                                    <span className={`px-2 py-0.5 rounded text-xs ${task.status === 'จบงานแล้ว' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                                        {task.status}
                                    </span>
                                </div>
                            </div>
                    ...
                    <div className="flex justify-end mt-4">
                        <Button variant="outline" onClick={() => setActiveGroup(null)}>Close</Button>
                    </div>
                </DialogContent>
            </Dialog>
        </>
