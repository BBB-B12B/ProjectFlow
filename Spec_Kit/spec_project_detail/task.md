...
- [x] [T-192] **Add Date-Range Filter to Gantt Charts**: เพิ่มตัวกรองช่วงวันที่ (Start/End) ในแท็บ Gantt ทั้งหน้า Projects Dashboard (`/projects`) และ Project Detail (`/project/[id]`) — เดิมมีแค่ Monthly/Weekly (คุมความละเอียดแกนเวลา) แต่ไม่มี date range filter ทำให้ Gantt auto-scale แสดงทุกงาน/ทุกโปรเจกต์เสมอ กรองไม่ได้
  - **Type**: Feature
  - **Traceability**: [F-002], [F-003]
  - **Implementation**:
    1. **Root Cause**: แท็บ Gantt มีแค่ `timeframe` state (Monthly/Weekly คุมความละเอียดแกน) ไม่มี date-range state และส่ง `projects`/`tasks` แบบเต็ม (unfiltered) เข้า Gantt component ที่ auto-scale ตามข้อมูลทั้งหมด
    2. **Action**: เพิ่ม `ganttStartDate`/`ganttEndDate` state + `<Input type="date">` (Start/End) + ปุ่ม "ล้าง" ข้าง RadioGroup, กรองด้วย `useMemo` แบบ Overlap logic (`item.start <= filterEnd && item.end >= filterStart`) ก่อนส่งเข้า Gantt (`ganttProjects`/`ganttTasks`) — **Default = ปีปัจจุบัน** (`new Date().getFullYear()` → `YYYY-01-01` ถึง `YYYY-12-31`), กด "ล้าง" = ค่าว่าง = แสดงทั้งหมด
    3. **Axis Binding & Clipping**: ส่ง `rangeStart`/`rangeEnd` เข้า `ProjectGanttChart` + `TaskGanttChart` — เมื่อมีช่วงกรอง ให้ยึดแกนเวลา (min/max) ตามช่วงนั้นแทน auto-scale ตามข้อมูล และ **clip แท่ง** (`clampedStart`/`clampedEnd` + cap duration ไม่ให้เกินขอบแกน) เพื่อให้งานที่จบเลยขอบเขตถูกตัดจบที่ขอบ ไม่ลากยาวเต็มจอ
    4. **Default Start = วันแรกของเดือนปัจจุบัน** (ปรับจากเดิม 1 ม.ค.), End ยังคงเป็นสิ้นปีปัจจุบัน
    5. **Tooltip Enrichment**: Project tooltip เพิ่มบรรทัด "เริ่ม/จบ" (แยกบรรทัด) + "Progress" (คำนวณจาก `completedTasks/totalTasks`); Task tooltip เพิ่มบรรทัด "เริ่ม" (เดิมมีแค่ "จบ/Due") — ทั้งสอง tooltip ใช้ `rangeForTooltip` ซึ่งเป็นวันที่จริงของ item (ไม่ใช่ค่าที่ถูก clip เพื่อวาดกราฟ) เพื่อให้ผู้ใช้เห็นวันที่จริงแม้แท่งจะถูกตัดแสดงผลแค่ในกรอบที่กรอง
    6. **Tooltip Task List (Project Gantt)**: Project tooltip เพิ่มรายการ Task ทั้งหมดของ Project นั้น แสดงเป็น Card ย่อยแยกทีละตัว (ชื่อ + ช่วงวันที่ + Progress %) ต่อท้ายส่วนสรุป, ครอบด้วย container `max-h-56 overflow-y-auto` ให้ scroll ดูได้เมื่อมีหลายรายการ — ต้องเพิ่ม `wrapperStyle={{ pointerEvents: 'auto' }}` ให้ Recharts `<Tooltip>` (ปกติ wrapper มี `pointer-events: none` ทำให้ scroll ด้วยเมาส์ไม่ได้) และเพิ่ม `tasks?: Task[]` ใน `Project` type พร้อมแนบ `tasksForProject` จาก server component (`src/app/projects/page.tsx`) เข้าไปในแต่ละ Project ที่ส่งลงมา
       - **Bug พบระหว่างทดสอบ**: real-time `onSnapshot` listener ใน `projects-client-page.tsx` (บรรทัด ~163) rebuild project object ใหม่จาก Firestore แต่ดึงแค่ `completedTasks`/`totalTasks` จาก `initialProjectData` ไม่ได้ดึง `tasks` มาด้วย ทำให้ค่าที่ set ไว้ตอน mount ถูกเขียนทับเป็นค่าว่างภายในไม่กี่ร้อย ms — แก้โดยเพิ่ม `tasks: initialProjectData?.tasks || []` ในจุดเดียวกับที่ copy `completedTasks`/`totalTasks`
    7. **Scroll-hijack Bug Fix**: หลังเปิดใช้ `pointerEvents: 'auto'` (ข้อ 6) พบว่าเวลาเลื่อนเมาส์/scroll ลงไปดู Task การ์ดที่อยู่ล่างๆ ใน tooltip เมาส์จะเคลื่อนทับพื้นที่ของแท่ง Gantt แถวถัดไป (เพราะ tooltip ลอยทับหลายแถว) ทำให้ event `mousemove` bubble ขึ้นไปถึง container ของ Recharts แล้วคำนวณ active bar ใหม่ กลายเป็นสลับไปแสดง tooltip ของ Project อื่นแทนกลางคัน — แก้โดยเพิ่ม `onMouseMove`/`onMouseOver`/`onMouseEnter`/`onWheel` ที่เรียก `e.stopPropagation()` บน root ของ `CustomGanttTooltip` กัน event bubble ออกจาก tooltip ไปหา Recharts wrapper
    8. **Task Card Sort & Fade (100% Tasks)**: เรียง Task card ใน tooltip ใหม่ — งานที่ยังไม่เสร็จ (`Progress !== 100`) ขึ้นบนเสมอ เรียงจากวันจบ (`EndDate`) ใกล้ที่สุดก่อน (เหลือเวลาน้อยสุด = ต้องรีบทำ ขึ้นบนสุด); งานที่เสร็จแล้ว (`Progress === 100`) ตกไปด้านล่างสุดของรายการทั้งหมด และใช้สไตล์สีเทาจาง (`opacity-60`, `bg-muted/20`, `text-muted-foreground`) แยกจากงานที่ยังไม่เสร็จ (`bg-muted/40` ปกติ)
    9. **Popup Task-Gantt Detail Button**: เพิ่มปุ่ม (ไอคอน `Maximize2` มุมขวาบนของ tooltip) เปิด `<Dialog>` แสดง `TaskGanttChart` ของ Task ทั้งหมดในโปรเจกต์นั้นแบบเต็ม เพื่อให้ติดตามแผนงานละเอียดได้โดยไม่ต้องออกจากหน้า Dashboard — เก็บ state `detailProject` ไว้ที่ `ProjectGanttChart` (component แม่ ไม่ใช่ tooltip ชั่วคราว) ผ่าน `onOpenDetail` callback ที่ส่งเข้า `CustomGanttTooltip` เพื่อให้ popup ยังเปิดค้างอยู่ได้แม้เมาส์จะออกจากแท่ง Gantt แล้ว tooltip unmount ไปแล้วก็ตาม; ปุ่มมี `stopPropagation()` กันชนกับปัญหา scroll-hijack ในข้อ 7
    10. **Date-Range Slicer in Popup**: เพิ่มตัวกรองช่วงวันที่ (Start/End) เข้าไปใน Popup (ข้อ 9) ด้วย — ใช้เกณฑ์ default เดียวกับตัวกรองหลัก (`getDefaultRangeStart`/`getDefaultRangeEnd`: วันแรกของเดือนปัจจุบัน → สิ้นปีปัจจุบัน) และ logic กรอง/clip แบบเดียวกัน (Overlap filter + ส่ง `rangeStart`/`rangeEnd` เข้า `TaskGanttChart` เพื่อ clip แกน); state (`detailStartDate`/`detailEndDate`) รีเซ็ตกลับเป็นค่า default ทุกครั้งที่เปิด popup ใหม่ (ผ่าน `openDetail()`) เพื่อไม่ให้ค่ากรองเก่าของโปรเจกต์ก่อนหน้าเหลือค้าง มีปุ่ม "ล้าง" เหมือนตัวกรองหลัก
    11. **Row Sort by Start Date**: เรียงแถว (Y-axis) ของ Gantt ทั้งสองแบบตาม `startDate`/`StartDate` จากเร็วสุดไปช้าสุด ให้รายการที่เริ่มก่อนอยู่บนสุดเสมอ — เดิมไม่มีการเรียงเลย (ใช้ลำดับตามที่รับเข้ามาจาก props ตรงๆ) ทำให้แถวมั่วไม่สัมพันธ์กับเวลา (พบจากตัวอย่างจริง: โปรเจกต์ "KMS Content input" มี Task เรียงย้อนกลับ คือ "Follow UP" (เริ่ม Oct 1) อยู่บนสุด "Desing Thinking" (เริ่ม Jul 1, เร็วสุด) อยู่ล่างสุด) — แก้ที่ `ProjectGanttChart` (`sortedProjects` ก่อนสร้าง `chartData`) และ `TaskGanttChart` (`validTasks.sort(...)` หลัง filter วันที่ถูกต้อง) ทำให้ Popup Task-Gantt (ข้อ 9) ได้ผลเรียงถูกต้องไปด้วยโดยอัตโนมัติเพราะใช้ component เดียวกัน
    12. **Files**: `src/app/projects/projects-client-page.tsx`, `src/app/projects/page.tsx`, `src/components/project-details-client.tsx`, `src/components/project-gantt-chart.tsx`, `src/components/task-gantt-chart.tsx`, `src/lib/types.ts`
    13. **Verification**: `tsc --noEmit` ผ่านสำหรับไฟล์ที่แก้ + ทดสอบจริงบน dev server (Node 20) ทั้ง Dashboard และ Detail — Default Start = วันแรกของเดือนปัจจุบัน (ยืนยัน 2026-07-01), แท่งที่จบเลยขอบเขตถูก clip ที่ขอบ, hover tooltip แสดงชื่อ/เริ่ม/จบ (วันจริง)/Progress ถูกต้อง, hover Project bar ("Kaizen") แสดง Task ย่อยครบ 4 รายการเป็น Card พร้อม scroll ทำงานถูกต้อง, ยืนยันด้วย DOM-level test ว่า scroll ไปสุดรายการ (`scrollTop` เปลี่ยนค่าจริง) tooltip ยังคง lock อยู่กับ Project เดิม ไม่กระโดดสลับ, ทดสอบ Project "After Sale" (15/21 เสร็จ) — Task ที่ไม่เสร็จ 5 รายการเรียงตามวันจบใกล้สุดขึ้นก่อน (Apr 8 → May 8 → Jul 3 → Oct 30 → Dec 30) ตามด้วย Task ที่เสร็จ 100% ทั้ง 16 รายการอยู่ด้านล่างพร้อมสไตล์จางถูกต้อง, คลิกปุ่มมุมขวาบนของ tooltip → Dialog เปิดแสดง Task Gantt ของ Project นั้นถูกต้องพร้อม date slicer แสดงค่า default `2026-07-01`–`2026-12-31` ตรงกับตัวกรองหลัก และมีปุ่ม "ล้าง" ครบ, ทดสอบ sort โปรเจกต์ "KMS Content input" ก่อนแก้เรียง Follow UP(Oct1)→...→Desing Thinking(Jul1) หลังแก้เรียงถูกต้อง Desing Thinking(Jul1)→Develop System(Jul7)→Content Create(Jul8)→Implement Department(Sep1)→Follow UP(Oct1), ยืนยันแถว Project หลักบน Dashboard เรียงจาก Aug 26 2025 → ... → Jul 8 2026 ถูกต้องเช่นกัน (มี pre-existing errors 2 จุดใน `project-gantt-chart.tsx` (status comparison) และ `theme-provider.tsx:5` ที่ไม่เกี่ยวกับงานนี้)
    14. **Status**: Done
